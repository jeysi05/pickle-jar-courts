  ...
  return (
    <div className={`min-h-screen relative selection:bg-lime-500/30 overflow-x-hidden ${isCoachMode ? 'bg-zinc-950' : 'bg-zinc-950'}`}>
      ...
      <div className="relative z-10 pb-20">
        ...
        <nav className="border-b border-white/5 backdrop-blur-sm sticky top-0 z-50 bg-zinc-950/80">
          <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Full Name Visible, but smaller on mobile (text-sm) */}
              <span className="font-bold text-white text-sm md:text-xl tracking-tight leading-none">
                Pickle<span className={isCoachMode ? 'text-yellow-400' : 'text-lime-400'}>JarCourts</span>
                {isCoachMode && <span className="hidden md:inline ml-2 text-[10px] bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded uppercase tracking-widest border border-yellow-500/30">Coach</span>}
              </span>
            </div>
            ...
            <div className="flex gap-2 md:gap-4">
              {isCoachMode ? (
                 <button 
                   ...
                   className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-red-400 bg-red-500/10 px-3 py-2 rounded-full border border-red-500/20"
                 >
                ...
                <button 
                  ...
                  className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-zinc-500 hover:text-yellow-400 transition-colors uppercase tracking-widest bg-zinc-900/50 px-3 py-2 rounded-lg border border-white/5"
                >
              ...
              <button 
                ...
                className="text-[10px] md:text-xs font-bold text-zinc-500 hover:text-lime-400 transition-colors uppercase tracking-widest flex items-center gap-2 bg-zinc-900/50 px-3 py-2 rounded-lg border border-white/5"
              >
        ...
        {/* HERO SECTION - Resized Text */}
        <div className="pt-10 md:pt-16 pb-12 md:pb-24 text-center px-4">
          ...
          <div className="flex items-center justify-center gap-2 text-zinc-400 mb-12 font-medium text-sm">
            <MapPin className={`w-4 h-4 ${isCoachMode ? 'text-yellow-400' : 'text-lime-400'}`} />
            <span>PDR Business Hub, Cabuyao</span>
          </div>

          {/* GRID LAYOUT */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 max-w-7xl mx-auto px-2">
        ...
        <div className="text-center mt-12 border-t border-white/5 pt-12">
          <p className="text-zinc-600 text-xs uppercase tracking-widest">© 2026 Pickle Jar Courts</p>
        </div>
      </div>
    </div>
  );
