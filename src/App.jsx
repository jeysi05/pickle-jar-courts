...
const calculateBlockPrice = (startTime, duration, dateString, isCoach) => {
  ...
  const isFriSun = day === 0 || day === 5 || day === 6;
  const endTime = startTime + duration;

  if (isCoach) {
    let ratePerHour = 380; 
    if (isMonThurs && startTime >= 8 && startTime < 24) ratePerHour = 250;
    else if (isFriSun && startTime >= 8 && endTime <= 12) ratePerHour = 250;
    return ratePerHour * duration;
  } 

  if (isMonThurs && duration === 3 && startTime >= 10 && endTime <= 22) return 1000;
  return 380 * duration;
};
...
function App() {
  ...
  // Helper for footer time display
  ...
  const handleSlotClick = (courtId, timeSlot, date) => {
    // Directly lock in the time selected (which could now be X.5)
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-center gap-6 animate-in slide-in-from-bottom duration-300">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-white font-black text-xl italic uppercase tracking-tighter">
                                        Court {currentSelection.court} <span className="text-zinc-600">@</span> {formatTime(currentSelection.time)}
                                    </p>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="text-right">
                                    <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Block Price</p>
